...
unsigned int goldage_driver_poll(struct file * filp,struct poll_table_struct * wait){
	unsigned int mask = 0;
	struct scull_device * dev = filp->private_data;

	/* 17. Add wait queue header to poll_table_struct.*/
	poll_wait(filp,&dev->rq,wait);
	poll_wait(filp,&dev->wq,wait);

	/* 18.Judge the resource state and return the respond mask.*/
